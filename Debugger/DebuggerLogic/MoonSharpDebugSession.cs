...
namespace MoonSharp.VsCodeDebugger.DebuggerLogic
{
	internal class MoonSharpDebugSession : DebugSession, IAsyncDebuggerClient
	{
...
#else
			SendText("Connected to Sinespace (MoonSharp {0}) on process {2} (PID {3})",
					 Script.VERSION,
					 Script.GlobalOptions.Platform.GetPlatformName(),
	    ...
	    public void OnException(SyntaxErrorException ex)
	    {
	        SendText("syntax error : {0}", ex.DecoratedMessage);
	    }
